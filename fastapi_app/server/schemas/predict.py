...
class PredictRequest(BaseModel):
    ...
    education_num: int = Field(
        ..., description="Числовой уровень образования", alias="education-num"
    )
    marital_status: str = Field(
        ..., description="Семейное положение", alias="marital-status"
    )
    ...
    hours_per_week: int = Field(
        ..., description="Часов работы в неделю", alias="hours-per-week"
    )
    native_country: str = Field(
        ..., description="Страна происхождения", alias="native-country"
    )
...
class PredictResponse(BaseModel):
    ...
    proba: float = Field(
        ..., ge=0.0, le=1.0, description="Вероятность принадлежности к классу 1"
    )

    class Config:
        json_schema_extra = {
            "example": {"model_used": "Random Forest", "predict": 1, "proba": 0.68}
        }
